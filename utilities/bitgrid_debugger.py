...
def debug_bitgrid_realtime(hsv_roi, LUT=None, color_names=None, rows=ROWS, cols=COLS, window_name='Bitgrid Debug'):
    ...
    cv2.waitKey(1)  # short delay to refresh window
...
if __name__ == '__main__':
    # quick self-test: if debug_hsv_roi.npy exists, run debug
    if os.path.exists('debug_hsv_roi.npy'):
        print('Found debug_hsv_roi.npy — running visual debugger...')
        hsv = np.load('debug_hsv_roi.npy')
        try:
        ...
        stats = debug_bitgrid(hsv, LUT=LUT, color_names=color_names)
        print('Done.')
    else:
        print('No debug_hsv_roi.npy found — place a saved hsv ROI named debug_hsv_roi.npy and run again.')
